...
public class DistributedTask implements Callable<Void> {
	...
	private net.preibisch.bigdistributor.algorithm.task.BlockTask mainTask;
	
	private Metadata md;
	private KafkaManager manager;

	public DistributedTask(net.preibisch.bigdistributor.algorithm.task.BlockTask task) {
		this.mainTask = task;
	}
	@Override
