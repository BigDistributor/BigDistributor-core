...
public class IOTools {
	...
	static Log logger = Log.getLogger(MethodHandles.lookup().lookupClass().getSimpleName());
...
//		fi.info = imp.getInfoProperty();
		...
		try {
			...
			logger.error(new Date(System.currentTimeMillis()) + ": ERROR: Cannot save file '" + f.getAbsolutePath() + "':" + e);
			return false;
		} finally {
			if (out != null)
				try {
