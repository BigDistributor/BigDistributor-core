    ...
    public ArrayList<Block> divideIntoBlocks(final long[] imgSize, final long[] kernelSize) {
        ...
        for (int d = 0; d < numDimensions; ++d) {
            ...
            if (effectiveSizeGeneral[d] <= 0) {
                logger.info("Blocksize in dimension " + d + " (" + blockSize[d] + ") is smaller than the kernel (" + kernelSize[d] + ") which results in an negative effective size: " + effectiveSizeGeneral[d] + ". Quitting.");
                return null;
            }
